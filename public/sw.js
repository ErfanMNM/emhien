...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'UPDATE_ALARMS') {
    ...
    // Tự động bắt đầu check nếu chưa chạy
  ...
  // Respond to request for alarms data
  if (event.data && event.data.type === 'REQUEST_ALARMS_DATA') {
...
async function checkAlarms() {
  ...
  events.forEach(event => {
    ...
    if (now >= triggerTime && now <= event.timestart + 300) {
      ...
      // Mark as notified in local data
      if (!alarmsData.notifiedEvents) {
        alarmsData.notifiedEvents = [];
      }
      if (!alarmsData.notifiedEvents.includes(event.id)) {
        alarmsData.notifiedEvents.push(event.id);
      }
